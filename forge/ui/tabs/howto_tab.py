...
_INTRO = (
    "Forge 는 한 윈도우에서 3 가지 작업을 제공합니다.\n"
    "  • 탭 ① 개별 작업 — 한/글에서 작업 중인 문서에 정형 룰 1개씩 즉시 적용 (실시간)\n"
    "  • 탭 ② 양식삽입 — 고정된 보고서 양식(헤더·박스 8 종)을 커서 위치에 삽입\n"
    "  • 탭 ③ 마크다운 입력 — 개조식 markdown 을 새 .hwpx 파일로 변환 (배치)"
)
...
_QUICKSTART = [
    ...
    ("3. 탭 ①·②·③ 중 원하는 작업 시작",
     "버튼이나 단축키로 적용. 결과는 한/글 화면에서 즉시 확인."),
]
...
_TAB2 = (
    "고정된 보고서 양식 8 종을 활성 한/글 문서 커서 위치에 클릭 1 회로 삽입합니다.\n"
    "구성: 헤더 3 종 (메타헤더·중제목·소제목) / 참고박스 4 종 / 붙임박스 1 종.\n"
    "글자(`◆◆◆`, `◎◎◎`, '진행상황' 등) 는 placeholder — 사용자가 한/글에서 직접 교체."
)

_TAB3 = (
    "좌측에 개조식 markdown 을 붙여넣고 '변환' 클릭 → 새 .hwpx 산출.\n"
    "Forge 는 GitHub style `#` 헤더가 아닌 '개조식' 입력 — 자세한 형식은 spec/markdown-spec.md 참조.\n"
    "탭 상단의 양식 spec (페이지 여백·줄간격) 도 여기서 편집."
)
...
class HowToTab:
    def __init__(self, parent: tk.Misc):
        ...
        # 탭 ② 안내
        t2 = TtkLabelFrame(c, text="탭 ② 양식삽입 — 보고서 양식 1-클릭 삽입", padding=12)
