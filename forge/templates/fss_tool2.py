...
# ============================================================================
# 9~11. 기호 — 단순 InsertText 1~2 글자. IME 변환 없이 1 클릭 입력.
#
# tool2 디컴파일 카탈로그에 전용 메서드는 없으나, tool2 GUI 가 일반 `문장()`
# 헬퍼(=InsertText)로 ※/†/「」 를 박는 동일한 패턴. 폰트·자간 mutation 없음.
# ============================================================================

def 당구장(hwp: Any) -> None:
    """※ 1 글자 — 한국 공문서의 표준 주석 마커."""
    p.insert_text(hwp, "※")


def 십자가(hwp: Any) -> None:
    """† 1 글자 — 보조 주석 마커."""
    p.insert_text(hwp, "†")


def 꺽쇠_홑(hwp: Any) -> None:
    """「」 홑낫표 쌍 — 맑은 고딕으로 박고 typing attr 복귀.

    동작 순서:
      1. 입력 직전 typing attr 의 7 면 face/type 백업
      2. 맑은 고딕으로 변경 + 「」 박기 (두 글자 모두 맑은 고딕)
      3. 백업한 7 면 복원 — 이후 사용자 타이핑은 원래 폰트로

    캐럿은 」 뒤에 그대로. 가운데 내용은 사용자가 한/글에서 직접 입력
    (좌/우 화살표로 「」 사이로 이동 후 본인 원하는 폰트로 타이핑).
    """
...
def 꺽쇠_겹(hwp: Any) -> None:
    """『』 겹낫표 쌍 — 책·문헌 제목 강조용. 「」 와 동일 패턴 (캐럿 」 뒤)."""
    with p.temp_font_face(hwp, "맑은 고딕"):
        p.insert_text(hwp, "『』")
...
# group: 헤더 / 참고박스 / 붙임박스 / 기호 — UI 가 이 키 변경 지점마다 separator 삽입
금감_TEMPLATES: list[tuple[int, str, Callable, str, str, int]] = [
    ...
    # ── 기호 (단순 InsertText. 꺽쇠는 맑은 고딕으로 박음) ──
    (9,  "기호",   당구장,    "당구장 ※",       "주석 마커 ※ 1 글자",                  0),
    (10, "기호",   십자가,    "십자가 †",       "보조 주석 마커 † 1 글자",              0),
    (11, "기호",   꺽쇠_홑,   "꺽쇠 「」 (홑)",  "맑은 고딕 「」 박고 폰트 복귀. 내용은 직접 입력", 0),
    (12, "기호",   꺽쇠_겹,   "꺽쇠 『』 (겹)",  "맑은 고딕 『』 박고 폰트 복귀. 책·문헌 제목", 0),
]
